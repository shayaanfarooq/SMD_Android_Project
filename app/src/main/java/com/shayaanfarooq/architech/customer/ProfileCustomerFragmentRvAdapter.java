...
public class ProfileCustomerFragmentRvAdapter extends RecyclerView.Adapter<ProfileCustomerFragmentRvAdapter.MyViewHolder>{
    ...
    public ProfileCustomerFragmentRvAdapter(ArrayList<FloorPlanRecyclerObject> listOfFloorPlans, Context context)
    {
...
//        holder.coverImage.setImageResource(resIdCoverImage);
        if(listOfFloorPlans.get(position).getPaths()!=null) {
            if (listOfFloorPlans.get(position).getPaths().size() != 0) {
                Picasso.get().load(listOfFloorPlans.get(position).getPaths().get(0)).fit().centerCrop().placeholder(R.drawable.progress_animation).into(holder.coverImage);
            }
        }
